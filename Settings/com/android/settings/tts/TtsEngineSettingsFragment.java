...
public class TtsEngineSettingsFragment extends SettingsPreferenceFragment implements OnPreferenceClickListener, OnPreferenceChangeListener {
    ...
    private final BroadcastReceiver mLanguagesChangedReceiver = new C05392();
    private ListPreference mLocalePreference;
    private int mSelectedLocaleIndex = -1;
    private TextToSpeech mTts;
    private final OnInitListener mTtsInitListener = new C05381();
    private Intent mVoiceDataDetails;

    class C05381 implements OnInitListener {
        ...
        public void onInit(int status) {
            if (status != 0) {
                ...
                TtsEngineSettingsFragment.this.getActivity().runOnUiThread(new C05371());
            }
        }
    }

    class C05392 extends BroadcastReceiver {
        C05392() {
        }
    ...
    class C05403 implements Comparator<Pair<String, Locale>> {
        C05403() {
        }
    ...
    private void updateVoiceDetails(Intent data) {
        ...
        if (getPackageManager().queryIntentActivities(new Intent("android.intent.action.VIEW", Uri.parse("market://search?q=dummy")), 0).size() == 0) {
    ...
    private void updateDefaultLocalePref(ArrayList<String> availableLangs) {
        ...
        Collections.sort(entryPairs, new C05403());
