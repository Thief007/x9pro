...
public class BatteryHistoryChart extends View {
    final int H_OFFSET = ((int) getContext().getResources().getDimension(R.dimen.battery_history_chart_custom));
    ...
    int mBatteryCriticalLevel = this.mContext.getResources().getInteger(17694797);
    final Paint mBatteryCriticalPaint = new Paint(1);
    final Paint mBatteryGoodPaint = new Paint(1);
    int mBatteryLevel;
    int mBatteryWarnLevel = this.mContext.getResources().getInteger(17694799);
    ...
    static class ChartData {
        ...
        void draw(Canvas canvas, int top, int height) {
            ...
            for (int i = 0; i < this.mNumTicks; i++) {
                ...
                if (lastBin != 0) {
                    if (lastBin >= this.mPaints.length) {
                        lastBin = this.mPaints.length - 1;
                    }
...
*/
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
                ...
                case com.android.setupwizardlib.R$styleable.SuwSetupWizardLayout_suwIllustration /*5*/:
                    ...
                    break;
                case com.android.setupwizardlib.R$styleable.SuwSetupWizardLayout_suwIllustrationAspectRatio /*6*/:
                    dx = a.getFloat(attr, 0.0f);
                    break;
                case com.android.setupwizardlib.R$styleable.SuwSetupWizardLayout_suwIllustrationHorizontalTile /*7*/:
                    dy = a.getFloat(attr, 0.0f);
                    break;
                case com.android.setupwizardlib.R$styleable.SuwSetupWizardLayout_suwIllustrationImage /*8*/:
    ...
    void setStats(BatteryStats stats, Intent broadcast) {
        ...
        this.mWifiRunningLabel = getContext().getString(R.string.battery_stats_wifi_running_label);
        ...
        if (this.mBatteryBroadcast.getIntExtra("plugged", 0) == 0) {
            ...
            if (drainTime > 0) {
                ...
                this.mChargeLabelString = getContext().getResources().getString(R.string.power_discharging_duration, new Object[]{batteryPercentString, timeString});
            } else {
            ...
            if (chargeTime <= 0 || status == 5) {
                this.mChargeLabelString = getContext().getResources().getString(R.string.power_charging, new Object[]{batteryPercentString, statusLabel});
            } else {
                int resId;
                ...
                this.mChargeLabelString = getContext().getResources().getString(resId, new Object[]{batteryPercentString, timeString});
            }
        }
        this.mDrainString = "";
        ...
        this.mHaveCamera = (4194304 & aggrStates2) != 0;
                ...
                while (this.mStats.getNextHistoryLocked(rec) && i2 < N) {
                    if (rec.isDeltaData()) {
                        ...
                        if (this.mLargeMode) {
                            ...
                            boolean cameraOn = (rec.states2 & 4194304) != 0;
                            if (cameraOn != z5) {
                                if (cameraOn) {
    ...
    void drawChart(Canvas canvas, int width, int height) {
        ...
        if (this.mTimeLabels.size() > 1) {
            ...
            while (i < this.mTimeLabels.size()) {
                ...
                if (i == 0) {
                    x = label.f12x - (label.width / 2);
                    if (x < 0) {
                        x = 0;
                    }
                    canvas.drawText(label.label, (float) x, (float) y, this.mTextPaint);
                    canvas.drawLine((float) label.f12x, (float) ytick, (float) label.f12x, (float) (this.mThinLineWidth + ytick), this.mTextPaint);
                    lastX = x + label.width;
        ...
        if (this.mHavePhoneSignal) {
            this.mPhoneSignalChart.draw(canvas, ((height - this.mPhoneSignalOffset) - (this.mLineWidth / 2)) + this.H_OFFSET, this.mLineWidth);
        }
        ...
        if (this.mDateLabels.size() > 0) {
            ...
            for (i = this.mDateLabels.size() - 1; i >= 0; i--) {
                ...
                int left = label2.f11x - this.mThinLineWidth;
                x = label2.f11x + (this.mThinLineWidth * 2);
                if (label2.width + x >= lastLeft) {
                    x = (label2.f11x - (this.mThinLineWidth * 2)) - label2.width;
                ...
                if (left >= this.mLevelLeft) {
                    ...
                    this.mDateLinePath.moveTo((float) label2.f11x, (float) ytop);
                    this.mDateLinePath.lineTo((float) label2.f11x, (float) ybottom);
