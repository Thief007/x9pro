...
public class FlingAnimationUtils {
    ...
    public FlingAnimationUtils(Context ctx, float maxLengthSeconds) {
        ...
        this.mFastOutSlowIn = AnimationUtils.loadInterpolator(ctx, 17563661);
        this.mFastOutLinearIn = AnimationUtils.loadInterpolator(ctx, 17563663);
    ...
    private AnimatorProperties getProperties(float currValue, float endValue, float velocity, float maxDistance) {
        ...
        if (durationSeconds <= maxLengthSeconds) {
            ...
            this.mAnimatorProperties.interpolator = new InterpolatorInterpolator(new VelocityInterpolator(durationSeconds, velAbs, diff), this.mLinearOutSlowIn, this.mLinearOutSlowIn);
        } else {
            durationSeconds = maxLengthSeconds;
    ...
    private AnimatorProperties getDismissingProperties(float currValue, float endValue, float velocity, float maxDistance) {
        ...
        if (durationSeconds <= maxLengthSeconds) {
            ...
            durationSeconds = maxLengthSeconds;
            this.mAnimatorProperties.interpolator = new InterpolatorInterpolator(new VelocityInterpolator(durationSeconds, velAbs, diff), mLinearOutFasterIn, this.mLinearOutSlowIn);
        } else {
            durationSeconds = maxLengthSeconds;
