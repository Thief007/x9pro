...
public class DozeParameters {
    ...
    public static class PulseSchedule {
        ...
        public long getNextTime(long now, long notificationTime) {
            for (int i : this.mSchedule) {
                long time = notificationTime + ((long) (i * EVENT.DYNAMIC_PACK_EVENT_BASE));
                if (time > now) {
                    return time;
                }
            }
    ...
    private int getInt(String propName, int resId) {
        return MathUtils.constrain(SystemProperties.getInt(propName, this.mContext.getResources().getInteger(resId)), 0, 10000);
    }
