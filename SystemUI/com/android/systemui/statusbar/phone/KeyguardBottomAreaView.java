...
public class KeyguardBottomAreaView extends FrameLayout implements OnClickListener, OnUnlockMethodChangedListener, AccessibilityStateChangedCallback, OnLongClickListener {
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        this.mIndicationText.setTextSize(0, (float) getResources().getDimensionPixelSize(17105137));
    ...
    private void updateCameraVisibility() {
        if (this.mCameraImageView != null) {
            boolean visible;
            int i;
            ResolveInfo resolved = this.mContext.getPackageManager().resolveActivityAsUser(getCameraIntent(), 65536, KeyguardUpdateMonitor.getCurrentUser());
    ...
    public void bindCameraPrewarmService() {
        ActivityInfo targetInfo = PreviewInflater.getTargetActivityInfo(this.mContext, getCameraIntent(), KeyguardUpdateMonitor.getCurrentUser());
        if (targetInfo != null && targetInfo.metaData != null) {
            String clazz = targetInfo.metaData.getString("android.media.still_image_camera_preview_service");
            if (clazz != null) {
                ...
                try {
                    if (getContext().bindServiceAsUser(serviceIntent, this.mPrewarmConnection, 1, new UserHandle(-2))) {
    ...
    private void launchVoiceAssist() {
        ...
        if (this.mPhoneStatusBar.isKeyguardCurrentlySecure()) {
            ...
            this.mPhoneStatusBar.executeRunnableDismissingKeyguard(runnable, null, false, false);
        }
    }
    ...
    public void startFinishDozeAnimation() {
        ...
        this.mIndicationText.animate().alpha(1.0f).setInterpolator(this.mLinearOutSlowInInterpolator).setDuration(700);
    }

    private void startFinishDozeAnimationElement(View element, long delay) {
        element.setAlpha(0.0f);
        element.setTranslationY((float) (element.getHeight() / 2));
        element.animate().alpha(1.0f).translationY(0.0f).setInterpolator(this.mLinearOutSlowInInterpolator).setStartDelay(delay).setDuration(250);
    }
