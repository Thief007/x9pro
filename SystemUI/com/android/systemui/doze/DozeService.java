...
public class DozeService extends DreamService {
    ...
    private abstract class ProximityCheck implements SensorEventListener, Runnable {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            finishWithResult((event.values[0] > this.mMaxRange ? 1 : (event.values[0] == this.mMaxRange ? 0 : -1)) < 0 ? 1 : 2);
        }
    ...
    private class TriggerSensor extends TriggerEventListener {
        ...
        public void onTrigger(TriggerEvent event) {
            ...
            try {
                ...
                this.mRegistered = false;
                updateListener();
                boolean withinVibrationThreshold = System.currentTimeMillis() - DozeService.this.mNotificationPulseTime < ((long) DozeService.this.mDozeParameters.getPickupVibrationThreshold());
    ...
    private void requestPulse(int reason) {
        if (!(this.mHost == null || !this.mDreaming || this.mPulsing)) {
            ...
            if (this.mDozeParameters.getProxCheckBeforePulse()) {
                boolean pickupPerformsProxCheck;
                final long start = SystemClock.uptimeMillis();
                if (reason == 3) {
                    pickupPerformsProxCheck = this.mDozeParameters.getPickupPerformsProxCheck();
                } else {
                    pickupPerformsProxCheck = false;
                }
    ...
    private void updateNotificationPulse(long notificationTimeMs) {
        ...
        if (notificationTimeMs - this.mNotificationPulseTime < ((long) this.mDozeParameters.getPulseDuration(false))) {
            if (DEBUG) {
